...
async def massive_scan():
    ...
    try:
        ...
        print(f'⏱️  Scan time: {(datetime.now() - start_time).total_seconds():.1f} seconds')
        ...
        for i, alert in enumerate(all_alerts[:25]):
            ...
            print(f'{i+1:2d}. {alert["base_symbol"]}/{alert["quote_symbol"]} | {alert["dex_name"]}')
            print(f'     💰 ${alert["liquidity_usd"]:,.0f} liq | 📈 ${alert["volume_24h_usd"]:,.0f} vol | ⏰ {age_str}')
            print(f'     🎯 {alert["opportunity_score"]:.2f} opp | 🛡️  {alert["safety_score"]:.2f} safe | 🚀 {alert["momentum_score"]:.2f} mom')
            print(f'     📊 {alert["price_change_24h"]:+.1f}% 24h | 🔄 {alert["txns_24h"]} txns')
            print()
        ...
        if len(all_alerts) > 200:
            ...
            print(f'This scanner is finding more opportunities than DexScreener itself!')
