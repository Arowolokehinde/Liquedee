...
class AlphaScanner:
    ...
    async def scan_alpha_gems(self, max_gems: int = 20) -> List[Dict]:
        ...
        """
        logger.info(f"🔥 Starting alpha gem scan across chains (target: {max_gems} gems)...")
        ...
        try:
            ...
            if len(all_alpha_gems) < max_gems:
                spike_leaders = await self._scan_price_spike_leaders(max_gems - len(all_alpha_gems))
            ...
            if len(all_alpha_gems) < max_gems:
                social_trending = await self._scan_social_trending(max_gems - len(all_alpha_gems))
                all_alpha_gems.extend(social_trending)
                logger.info(f"🐦 Social Trending: Found {len(social_trending)} alpha gems")
            
            # Strategy 4: New listings with momentum
            if len(all_alpha_gems) < max_gems:
                momentum_gems = await self._scan_momentum_gems(max_gems - len(all_alpha_gems))
            ...
            logger.info(f"🔥 Alpha gem scan complete: {len(final_gems)} trending gems found")
            return final_gems
