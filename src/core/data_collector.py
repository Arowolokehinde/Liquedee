...
from ..database.models import TokenPair, LiquiditySnapshot, PairAlert
...
class DataCollector:
    ...
    async def _collect_cycle(self):
        ...
        for raw_pair in raw_pairs:
            try:
                ...
                logger.error(f"Error processing pair {pair_data.get('pair_address', 'unknown')}: {e}")
    ...
    async def _store_pair_data(self, pair_data: Dict) -> bool:
        ...
        try:
            ...
            existing_pair = session.query(TokenPair).filter_by(
    ...
    async def _store_alert(self, alert_data: Dict):
        ...
        try:
            ...
            logger.info(f"🚨 Alert generated: {alert_data['alert_type']} for {alert_data['pair_address']}")
