...
class MassiveDexScreenerClient:
    ...
    async def scan_massive_opportunities(self) -> List[Dict]:
        ...
        print(
            f"🎉 MASSIVE SCAN COMPLETE: {len(unique_opportunities)} TOTAL OPPORTUNITIES!"
        )
        ...
        try:
            ...
            return {
                ...
                "total_liquidity_usd": float(
                    raw_pair.get("liquidity", {}).get("usd", 0) or 0
                ),
                "volume_24h_usd": float(raw_pair.get("volume", {}).get("h24", 0) or 0),
                "price_usd": float(raw_pair.get("priceUsd", 0) or 0),
                "price_change_24h": float(
                    raw_pair.get("priceChange", {}).get("h24", 0) or 0
                ),
                "txns_24h": (
                    raw_pair.get("txns", {}).get("h24", {}).get("buys", 0) or 0
                )
