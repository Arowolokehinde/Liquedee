...
class GemHunterScanner:
    ...
    async def hunt_gems(self, max_gems: int = 15) -> List[Dict]:
        ...
        try:
            ...
            if len(all_gems) < max_gems:
                factory_gems = await self._hunt_dex_factory_gems(max_gems - len(all_gems))
            ...
            verified_gems = [gem for gem in unique_gems if self._verify_gem_criteria(gem)]
                ...
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        for pair in pairs[:50]:  # Check first 50 from each endpoint
                            ...
                            if gem and self._is_solana_token(pair) and self._is_potential_gem(gem):
                                gems.append(gem)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for pair in pairs[:30]:
                    ...
                    if gem and self._is_solana_token(pair) and self._is_ultra_fresh_gem(gem):  # Extra fresh criteria
                        gems.append(gem)
    ...
    def _parse_potential_gem(self, pair: Dict, source: str) -> Optional[Dict]:
        ...
        try:
            ...
            volume_spike = self._calculate_volume_spike(volume_1h, volume_24h, age_hours)
    ...
    def _calculate_volume_spike(self, volume_1h: float, volume_24h: float, age_hours: float) -> float:
    ...
    def _is_solana_token(self, pair: Dict) -> bool:
        ...
        try:
            ...
            solana_dexs = ['raydium', 'orca', 'jupiter', 'meteora', 'openbook', 'lifinity']
            
            # Check if it's explicitly marked as Solana chain or uses Solana DEXs
            if (chain_id == 'solana' or 
                any(dex in dex_id for dex in solana_dexs)):
                return True
            ...
            if (base_token.get('address') == sol_address or 
                quote_token.get('address') == sol_address):
                return True
    ...
    def _verify_gem_criteria(self, gem: Dict) -> bool:
        ...
        try:
            ...
            # Volume spike check
            if gem.get('volume_spike_percent', 0) < criteria['min_volume_spike_percent']:
                return False
            
            # Market cap range check
            mcap = gem.get('market_cap_usd', 0)
            if mcap > 0:  # Only check if we have market cap data
                if mcap < criteria['min_market_cap'] or mcap > criteria['max_market_cap']:
                    return False
    ...
    async def get_newest_tokens_fallback(self, max_tokens: int = 3) -> List[Dict]:
        ...
        try:
                        ...
                        for pair in pairs[:30]:
                            ...
                            if token:
                                ...
                                if (age_hours <= max_age_hours and 
                                    token.get('liquidity_usd', 0) >= 1000 and
                                    ('raydium' in dex_id or 'orca' in dex_id or 'jupiter' in dex_id or
                                     'chainId' in pair and pair.get('chainId') == 'solana')):
                                    fresh_tokens.append(token)
                                
                                if len(fresh_tokens) >= max_tokens * 2:  # Get more than needed
                                    break
            ...
            if fresh_tokens:
                ...
                fresh_tokens.sort(key=lambda x: (x.get('age_hours', 999), -x.get('gem_score', 0)))
                result = fresh_tokens[:max_tokens]
                
                logger.info(f"Fallback found {len(result)} fresh Solana tokens under {max_age_hours}h")
                return result
            else:
                logger.info(f"No fresh Solana tokens found under {max_age_hours}h for fallback")
                return []
