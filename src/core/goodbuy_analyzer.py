...
class GoodBuyAnalyzer:
    ...
    async def analyze_token_goodbuy(self, token_address: str) -> Dict:
        ...
        try:
            ...
            for result in [safety_result, market_result, momentum_result, distribution_result]:
            ...
            analysis['recommendation'], analysis['risk_level'] = self._generate_recommendation(analysis['overall_score'], analysis['red_flags'])
            
            logger.info(f"✅ GoodBuy analysis complete: {analysis['overall_score']:.1f}/10 - {analysis['recommendation']}")
    ...
    async def _analyze_safety(self, token_data: Dict) -> Dict:
        ...
        try:
            ...
            if liquidity_locked['locked_months'] >= 6:
                result['score'] += 3
                result['good_signs'].append(f"🔒 Liquidity locked for {liquidity_locked['locked_months']} months")
            elif liquidity_locked['locked_months'] > 0:
                result['score'] += 1
                result['warnings'].append(f"⚠️ Liquidity locked for only {liquidity_locked['locked_months']} months")
            else:
            ...
            if not suspicious_functions:
                ...
                result['red_flags'].append(f"🚨 Suspicious functions: {', '.join(suspicious_functions)}")
            ...
            if age_hours > 168:  # > 1 week
                result['score'] += 1
                result['good_signs'].append(f"📅 Token age: {age_hours/24:.1f} days (established)")
            elif age_hours > 24:  # > 1 day
                result['warnings'].append(f"⚠️ Token age: {age_hours:.1f} hours (new)")
            else:
                result['red_flags'].append(f"🚨 Token age: {age_hours:.1f} hours (very new)")
    ...
    async def _analyze_market_health(self, token_data: Dict) -> Dict:
        ...
        try:
            ...
            if 10000 <= market_cap <= 500000:
                ...
                result['warnings'].append(f"⚠️ Large market cap: ${market_cap:,.0f} (less upside)")
            else:
                result['red_flags'].append(f"🚨 Very small market cap: ${market_cap:,.0f}")
            ...
            if total_txns >= 100:
                result['score'] += 1
                result['good_signs'].append(f"📊 Active trading: {total_txns} transactions")
            elif total_txns >= 20:
                result['warnings'].append(f"⚠️ Moderate activity: {total_txns} transactions")
            else:
        ...
        try:
            ...
            if volume_24h > 0:
                ...
                if expected_hourly > 0:
                    ...
                    if volume_spike >= 300:
                        result['score'] += 3
                        result['good_signs'].append(f"📈 Massive volume spike: {volume_spike:.0f}%")
                    elif volume_spike >= 150:
                        result['score'] += 2
                        result['good_signs'].append(f"📈 Strong volume spike: {volume_spike:.0f}%")
                    elif volume_spike >= 50:
                        result['score'] += 1
                        result['warnings'].append(f"⚠️ Moderate volume spike: {volume_spike:.0f}%")
                    else:
                        result['red_flags'].append(f"🚨 No volume spike: {volume_spike:.0f}%")
            ...
            if sells > 0:
                ...
                if buy_sell_ratio >= 3:
                    result['score'] += 3
                    result['good_signs'].append(f"🟢 Excellent buy/sell ratio: {buy_sell_ratio:.1f}:1")
                elif buy_sell_ratio >= 2:
                    result['score'] += 2
                    result['good_signs'].append(f"🟢 Good buy/sell ratio: {buy_sell_ratio:.1f}:1")
                elif buy_sell_ratio >= 1:
                    result['score'] += 1
                    result['warnings'].append(f"⚠️ Balanced ratio: {buy_sell_ratio:.1f}:1")
                else:
                    result['red_flags'].append(f"🔴 Poor buy/sell ratio: {buy_sell_ratio:.1f}:1")
            ...
            if price_change_1h >= 30:
                result['score'] += 2
                result['good_signs'].append(f"🚀 Strong 1h trend: +{price_change_1h:.1f}%")
            elif price_change_1h >= 15:
                result['score'] += 1
                result['good_signs'].append(f"📈 Positive 1h trend: +{price_change_1h:.1f}%")
            elif price_change_1h >= 0:
                result['warnings'].append(f"⚠️ Flat 1h trend: {price_change_1h:+.1f}%")
            else:
                result['red_flags'].append(f"📉 Negative 1h trend: {price_change_1h:+.1f}%")
    ...
    async def _analyze_distribution(self, token_data: Dict) -> Dict:
        ...
        try:
            ...
            if top_wallet_percent <= 10:
                result['score'] += 3
                result['good_signs'].append(f"✅ Excellent distribution: Top wallet {top_wallet_percent:.1f}%")
            elif top_wallet_percent <= 20:
                result['score'] += 2
                result['good_signs'].append(f"✅ Good distribution: Top wallet {top_wallet_percent:.1f}%")
            elif top_wallet_percent <= 30:
                result['score'] += 1
                result['warnings'].append(f"⚠️ Moderate concentration: Top wallet {top_wallet_percent:.1f}%")
            else:
                result['red_flags'].append(f"🚨 High concentration: Top wallet {top_wallet_percent:.1f}%")
            ...
            if dev_wallet_percent <= 10:
                result['score'] += 2
                result['good_signs'].append(f"✅ Low dev allocation: {dev_wallet_percent:.1f}%")
            elif dev_wallet_percent <= 20:
                result['score'] += 1
                result['warnings'].append(f"⚠️ Moderate dev allocation: {dev_wallet_percent:.1f}%")
            else:
                result['red_flags'].append(f"🚨 High dev allocation: {dev_wallet_percent:.1f}%")
    ...
    def _calculate_overall_score(self, safety: float, market: float, momentum: float, distribution: float) -> float:
    ...
    def _generate_recommendation(self, overall_score: float, red_flags: List[str]) -> Tuple[str, str]:
        ...
        has_critical_flags = any(any(flag in red_flag for flag in critical_flags) for red_flag in red_flags)
