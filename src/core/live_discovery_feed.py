...
class LiveDiscoveryScanner:
    ...
    async def scan_live_discoveries(self, max_discoveries: int = 15) -> List[Dict]:
        ...
        """
        logger.info(f"🚀 Starting live discovery scan (target: {max_discoveries} discoveries)...")
        ...
        try:
            ...
            if len(all_discoveries) < max_discoveries:
                pump_tokens = await self._scan_fresh_pump_tokens(max_discoveries - len(all_discoveries))
                all_discoveries.extend(pump_tokens)
                logger.info(f"🚀 Pump Style: Found {len(pump_tokens)} discoveries")
            
            # Strategy 3: New DEX listings
            if len(all_discoveries) < max_discoveries:
                dex_tokens = await self._scan_new_dex_listings(max_discoveries - len(all_discoveries))
                all_discoveries.extend(dex_tokens)
                logger.info(f"🏭 DEX Listings: Found {len(dex_tokens)} discoveries")
            
            # Strategy 4: Trending searches
            if len(all_discoveries) < max_discoveries:
                trending_tokens = await self._scan_trending_tokens(max_discoveries - len(all_discoveries))
            ...
            filtered_discoveries = [d for d in unique_discoveries if self._meets_discovery_criteria(d)]
            
            # Sort by discovery score (freshness + activity)
            filtered_discoveries.sort(key=lambda x: x.get('discovery_score', 0), reverse=True)
            
            final_discoveries = filtered_discoveries[:max_discoveries]
            
            logger.info(f"🚀 Live discovery scan complete: {len(final_discoveries)} fresh discoveries found")
    ...
    def _parse_discovery(self, pair: Dict, source: str) -> Optional[Dict]:
        ...
        try:
            ...
            volume_spike = self._calculate_volume_activity(volume_1h, volume_24h, age_hours)
    ...
    def _calculate_volume_activity(self, volume_1h: float, volume_24h: float, age_hours: float) -> float:
    ...
    def _meets_discovery_criteria(self, discovery: Dict) -> bool:
        ...
        try:
            ...
            if discovery.get('volume_spike_percent', 0) < criteria['min_volume_spike_percent']:
                return False
