...
class RealtimeTokenSniffer:
        ...
        try:
            async with connect(self.wss_url) as websocket:
                ...
                await websocket.account_subscribe(
                    Pubkey.from_string(factory_address),
                ...
                try:
                    ...
                    logger.warning(f"Could not subscribe to logs for {dex_name}: {log_error}")
    ...
    async def _get_pair_details(self, pair_address: str, dex_name: str) -> Optional[Dict]:
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data.get('pair'):
                        ...
                        return self._format_pair_info(data['pair'], dex_name, from_blockchain=True)
            except Exception:
                pass  # DexScreener might not have indexed it yet
            
            # If not on DexScreener yet, get basic info from blockchain
            pair_info = await self._get_pair_info_from_blockchain(pair_address, dex_name)
            return pair_info
    ...
    async def _get_pair_info_from_blockchain(self, pair_address: str, dex_name: str) -> Optional[Dict]:
    ...
    def _format_pair_info(self, pair_data: Dict, dex_name: str, from_blockchain: bool = False) -> Dict:
        ...
        return {
            ...
            'dex_name': dex_name,
            'total_liquidity_usd': float(pair_data.get('liquidity', {}).get('usd', 0) or 0),
            ...
            'age_hours': age_hours,
            'discovery_method': 'blockchain_realtime' if from_blockchain else 'dexscreener_realtime',
            'discovery_time': discovery_time.isoformat(),
    ...
    async def _cleanup_old_pairs(self):
        ...
        while self.is_monitoring:
            try:
                cutoff_time = datetime.utcnow() - timedelta(hours=48)  # Keep 48h history
                
                pairs_to_remove = [
                    pair_addr for pair_addr, discovery_time in self.fresh_pairs.items()
                    if discovery_time < cutoff_time
                ]
...
class RealtimeSnifferFactory:
    ...
    @staticmethod
    def create_blockchain_sniffer(rpc_url: str = None, wss_url: str = None) -> RealtimeTokenSniffer:
