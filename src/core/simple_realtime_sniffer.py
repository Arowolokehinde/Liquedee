...
class SimpleRealtimeSniffer:
    ...
    async def _scan_recent_transactions(self, last_signature: Optional[str]) -> List[Dict]:
    ...
    async def _handle_new_pair_detected(self, pair_data: Dict):
        ...
        try:
            ...
            logger.info(f"🆕 NEW FRESH PAIR: {base_symbol} ({age_hours:.1f}h old) - {pair_address}")
    ...
    async def get_fresh_pairs_last_24h(self) -> List[Dict]:
        ...
        for pair_address, pair_data in self.fresh_pairs.items():
            ...
            try:
                ...
                discovery_time = discovery_time.replace(tzinfo=None)  # Remove timezone for comparison
