...
**🎯 Ready to snipe alpha? Try /quick now!**
    ...
    async def quick_scan_command(
        ...
        try:
            ...
            if opportunities:
                result_text = self.format_opportunities(
                    opportunities[: settings["max_alerts"]]
                )
    ...
    async def run_quick_scan(self, settings: Dict) -> List[Dict]:
        ...
        try:
            ...
            for raw_pair in pairs[: settings["max_scan_pairs"]]:
                ...
                if (
                    parsed["total_liquidity_usd"] >= settings["min_liquidity"]
                    and parsed["volume_24h_usd"] >= settings["min_volume"]
                ):
                    # Use full analyzer for proper scoring
                    alert = await analyzer.analyze_pair(parsed)
                    if (
                        alert
                        and alert.get("combined_score", 0) >= settings["min_confidence"]
                    ):
...
**🚀 Ready to snipe? Try /quick now!**
    ...
    async def alerts_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if self.last_opportunities:
            ...
            scan_time = (
                self.last_scan_time.strftime("%H:%M UTC")
    ...
    async def stats_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        fresh_count = (
            len([o for o in self.last_opportunities if o.get("age_hours", 999) <= 24])
            if self.last_opportunities
            else 0
        )
