...
class RealtimeSnifferBot:
    ...
    def setup_handlers(self):
        ...
        self.application.add_handler(
            CommandHandler("realtime", self.realtime_scan_command)
        )
        self.application.add_handler(
            CommandHandler("blockchain", self.blockchain_scan_command)
        )
        ...
        self.application.add_handler(
            CommandHandler("subscribe_realtime", self.subscribe_realtime_command)
        )
        self.application.add_handler(
            ...
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to send message after all retries: {final_error}"
                        )
                        return None
...
**🔥 Hunt smart, invest safer!**
    ...
    async def realtime_scan_command(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        ...
        try:
            ...
            discoveries = await self.live_discovery.scan_live_discoveries(
                max_discoveries=15
            )

            if discoveries:
                result_text = self.format_discovery_opportunities(
                    discoveries, "LiveFeed"
                )
    ...
    async def blockchain_scan_command(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
    ...
    async def handle_fresh_pair_discovery(self, pair_info: Dict):
        ...
        logger.info(
            f"🆕 Fresh pair discovered: {pair_info.get('base_symbol', 'UNKNOWN')}"
        )
            ...
            for user_id in self.realtime_subscribers.copy():
                try:
                    await self.application.bot.send_message(
                        chat_id=user_id, text=alert_text, parse_mode=ParseMode.MARKDOWN
                    )
                except Exception as e:
    ...
    def format_discovery_opportunities(
        self, discoveries: List[Dict], source: str
    ) -> str:
    ...
    def format_alpha_results(self, alpha_gems: List[Dict]) -> str:
        ...
        avg_gain = sum(gem.get("price_change_24h", 0) for gem in alpha_gems[:10]) / min(
            len(alpha_gems), 10
        )
        ...
        text += (
            f"🌐 Chains: {len(set(gem.get('chain', 'unknown') for gem in alpha_gems))}\n"
        )
    ...
    def format_single_opportunity(
        self, opp: Dict, source: str, index: int = None
    ) -> str:
        ...
        if freshness_score and combined_score:
            result += (
                f"   ⚡ {freshness_score:.2f} fresh | 🎯 {combined_score:.2f} total\n"
            )
    ...
    def format_single_discovery(
        self, discovery: Dict, source: str, index: int = None
    ) -> str:
            ...
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to edit message after all retries: {final_error}"
                        )
                        return None
...
**🔥 Hunt alpha, invest smart!**
    ...
    async def subscribe_realtime_command(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
    ...
    async def scan_disabled_command(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
        await self.safe_send_message(
            update, "🔄 Use /quick, /realtime, or /blockchain instead!"
        )

    async def alerts_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await self.safe_send_message(
            update, "📊 Use /quick or /realtime for fresh results!"
        )
    ...
    async def token_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        await self.safe_send_message(
            update, "🔍 Token analysis coming soon! Use /quick for opportunities."
        )
    ...
    async def goodbuy_help_callback(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ):
