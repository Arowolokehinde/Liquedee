    ...
    try:
        ...
        if alpha_gems:
            ...
            for i, gem in enumerate(alpha_gems[:5], 1):
                ...
                print(f"{i}. {gem.get('base_symbol', 'UNKNOWN')}/{gem.get('quote_symbol', 'UNKNOWN')} ({chain})")
                print(f"   🕐 {age_str} old | 💰 ${volume_24h:,.0f} volume | 📈 {price_change:+.1f}%")
        ...
        if alpha_gems:
            ...
            avg_gain = sum(gem.get('price_change_24h', 0) for gem in alpha_gems) / len(alpha_gems)
            
            print(f"\n📊 **ALPHA METRICS:**")
            print(f"• Total Volume: ${total_volume:,.0f}")
            print(f"• Average Gain: {avg_gain:+.1f}%")
            print(f"• Social Activity: {sum(gem.get('social_mentions', 0) for gem in alpha_gems)} mentions")
