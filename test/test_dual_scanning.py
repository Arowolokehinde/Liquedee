...
async def test_dual_scanning_system():
    ...
    try:
        ...
        discovery_task = asyncio.create_task(discovery_scanner.scan_live_discoveries(max_discoveries=15))
        ...
        if gems:
            ...
            for i, gem in enumerate(gems[:3], 1):
                ...
                print(f"   💰 ${gem.get('liquidity_usd', 0):,} liq | 📊 {gem.get('volume_spike_percent', 0):.0f}% spike")
                print(f"   💎 {gem.get('gem_score', 0):.1f}/10 gem score | 🏆 {gem.get('alert_type', 'N/A')}")
        else:
        ...
        if discoveries:
            ...
            for i, discovery in enumerate(discoveries[:3], 1):
                ...
                print(f"   💰 ${discovery.get('liquidity_usd', 0):,} liq | 📊 {discovery.get('volume_spike_percent', 0):.0f}% activity")
                print(f"   🚀 {discovery.get('discovery_score', 0):.1f}/10 discovery score | 🏷️ {discovery.get('discovery_type', 'N/A')}")
        else:
        ...
        if gems and discoveries:
            ...
            print(f"📊 QUIET MARKET: Both systems working, just no qualifying tokens right now")
