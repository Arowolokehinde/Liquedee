    ...
    try:
        ...
        if gems:
            ...
            for i, gem in enumerate(gems, 1):
                ...
                print(
                    f"{i}. {gem.get('base_symbol', 'UNKNOWN')}/{gem.get('quote_symbol', 'SOL')}"
                )
                print(
                    f"   🕐 {age_str} old | ✅ AGE CHECK: {'PASS' if age_hours <= 72 else 'FAIL'}"
                )
        ...
        if fallback_tokens:
            ...
            for i, token in enumerate(fallback_tokens, 1):
                ...
                print(
                    f"{i}. {token.get('base_symbol', 'UNKNOWN')}/{token.get('quote_symbol', 'SOL')}"
                )
                print(
                    f"   🕐 {age_str} old | ✅ AGE CHECK: {'PASS' if age_hours <= 72 else 'FAIL'}"
                )
        ...
        if all_tokens:
            ...
            if age_violations:
                ...
                for violation in age_violations:
                    ...
                    print(
                        f"   • {violation.get('base_symbol', 'UNKNOWN')}: {age_hours:.1f}h old"
                    )
            else:
            ...
            if non_solana_dexs:
                print(
                    f"❌ CHAIN VIOLATIONS: Non-Solana DEXs found: {set(non_solana_dexs)}"
                )
            else:
