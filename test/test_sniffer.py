...
def create_test_token_data(age_hours: float, liquidity: float, volume: float, txns: int = 50) -> dict:
...
def analyze_test_results(results):
    ...
    if fresh_accepted:
        avg_freshness = sum(r['freshness_score'] for r in fresh_accepted) / len(fresh_accepted)
        ...
        print(f"   Newest accepted: {newest_token['age_hours']}h (score: {newest_token['freshness_score']:.3f})")
        print(f"   Oldest accepted: {oldest_accepted['age_hours']}h (score: {oldest_accepted['freshness_score']:.3f})")
    
    # Overall assessment
    print(f"\n✅ SUCCESS CRITERIA:")
    print(f"   ✓ Rejects old tokens (>72h): {'✅' if len(old_accepted) == 0 else '❌'}")
    print(f"   ✓ Accepts some fresh tokens: {'✅' if len(fresh_accepted) > 0 else '❌'}")
    print(f"   ✓ Higher scores for newer tokens: {'✅' if check_freshness_correlation(fresh_accepted) else '❌'}")
